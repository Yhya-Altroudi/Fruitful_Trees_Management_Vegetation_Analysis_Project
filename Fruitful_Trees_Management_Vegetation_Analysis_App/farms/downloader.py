...
def main(left, top, right, bottom, zoom, filePath, style="s", server="Google China"):
    ...
    urls_group = [
        urls[i : i + math.ceil(len(urls) / multiprocessing.cpu_count())]
...
def f(left, top, right, bottom, zoom):
    style = "s"
    server = "Google"
    start_time = time.time()

    pos1x, pos1y = wgs_to_tile(left, top, zoom)
    pos2x, pos2y = wgs_to_tile(right, bottom, zoom)
    lenx = pos2x - pos1x + 1
    leny = pos2y - pos1y + 1
    print("Total tiles number：{x} X {y}".format(x=lenx, y=leny))
    urls = [
        get_url(server, i, j, zoom, style)
        for j in range(pos1y, pos1y + leny)
        for i in range(pos1x, pos1x + lenx)
    ]
    print("urls", urls)
    ...
    #     Xframe[i] = mercator_to_wgs(*Xframe[i])
    # pt = Xframe
    Xframe = (mercator_to_wgs(*Xframe["LT"]), mercator_to_wgs(*Xframe["RB"]))

    # Get the spatial information of the four corners of the merged map and use it for outputting
    inx, iny = (pos1x, pos1y)  # left top
    inx2, iny2 = (pos2x, pos2y)  # right bottom
    length = 20037508.3427892
    sum = 2**zoom
    LTx = inx / sum * length * 2 - length
    LTy = -(iny / sum * length * 2) + length
    ...
    return outpic, Xframe
    # return outpic, (pt["LT"], pt["RB"]), Xframe, ((LTx, LTy), (RBx, RBy))


# ((LTx, LTy), (RBx, RBy)), Xframe
...
if __name__ == "__main__":
    ...
    main(100.361, 38.866, 100.386, 38.839, 13, r"test.tif", server="Google")
